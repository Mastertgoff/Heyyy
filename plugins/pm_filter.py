...
async def next_page(bot, query):
    ...
    try:
        ...
        if settings['max_btn']:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("𝐍𝐄𝐗𝐓 ⌦", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    elif query.data.startswith("sendre_fall"):
        temp, ident, key, offset = query.data.split("#")
        search = BUTTONS.get(key)
        if not search:
            await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name),show_alert=True)
            return
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True) await send_all(client, query.from_user.id, files, ident)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if (AUTH_CHANNEL or REQ_CHANNEL) and not await is_subscribed(client, query):
                await query.answer(url=f"https://t.me/Elsaa_MsT_bot?start={ident}_{file_id}")
                return
            elif settings['botpm']:
                await query.answer(url=f"https://t.meElsaa_MsT_bot?start={ident}_{file_id}")
                return
            else:
    ...
    if imdb and imdb.get('poster'):
        try:
            if message.chat.id == SUPPORT_CHAT_ID:
                ...
                hehe = await message.reply_photo(photo="https://telegra.ph/file/dfea6bbfeb991f24cfa55.jpg", caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
                try:
                    if settings['auto_delete']:
